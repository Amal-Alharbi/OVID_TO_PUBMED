...
Usage: convert_OVED_PUBMED.py query_ovid.txt 
...
def Convert_OVID_To_PUBMED(q_OVID): 
    SubHeading_Dict = {}
...
def main():
    ...
    q_OVID = query.split('\n')
    q_con = Convert_OVID_To_PUBMED(q_OVID)
    q_Pub =convert_to_one_line('\n'.join(q_con))
    output = open('pubmed.txt','w+')    
    if 'limit' in list(q_Pub.values())[-1]:
        print(list(q_Pub.values())[-2])
        output.write(list(q_Pub.values())[-2])
    else:
        print(list(q_Pub.values())[-1])
        output.write(list(q_Pub.values())[-1])
    output.close()
